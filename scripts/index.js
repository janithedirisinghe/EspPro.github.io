...
function epochToDateTime(epochTime){
  ...
  function calculateTankVolume(length, radius, height) {
    ...
    var volume = (Math.sqrt(2 * height * radius - Math.pow(height, 2)) * (height - radius)) + (Math.pow(radius, 2) * (Math.PI - Math.acos((height - radius) / radius))) * length;
    vol=volume;
    return volume;
    }
  ...
  const setupUI = (user) => {
    if (user) {
      ...
      dbRef.orderByKey().limitToLast(1).on('child_added', snapshot =>{
        ...
        humElement.innerHTML = vol.toFixed(2);
